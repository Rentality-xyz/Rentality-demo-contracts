...
async function main() {
  const { contractName, chainId } = await startDeploy('RentalityClaimService')
  ...
  const contract = await upgrades.deployProxy(contractFactory, [userService, notificationService])
