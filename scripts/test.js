...
async function main() {
  ...
  let contract =  await contractFactory.deploy(poolManager, weth)
...
let updatedData = abiCoder.encode(['bytes', 'address', 'uint256'], [data, deployer.address, amountIn])
   let config = {
  poolKey: {
      currency0: linkToken,
      currency1: ethToken,
