...
describe('RentalityGateway: user info', function () {
  ...
  it('Should host be able to create KYC', async function () {
    ...
    await expect(
      rentalityGateway
        .connect(host)
        .setKYCInfo(name, number, photo, kyc, hostSignature,adminSignature)
    ).not.be.reverted
  ...
  it('Should guest be able to create KYC', async function () {
    ...
    await expect(
      rentalityGateway
        .connect(guest)
        .setKYCInfo(name, number, photo, kyc, guestSignature,adminSignature)
          ).not.be.reverted
  ...
  it('Guest should be able to get trip contacts', async function () {
    ...
    await expect(
      rentalityGateway
        .connect(guest)
        .setKYCInfo('name',  guestNumber, 'photo', emptyKyc,guestSignature,adminKyc)
    ).not.be.reverted
