...
describe('Rentality investment', function () {
  ...
  it('Host can create investment', async function () {
    ...
    await expect(await rentalityGateway.connect(host).createCarInvestment(mockCarInvestment, 'name',ethToken)).to.not
      .reverted
  })
  it('Guest can invest', async function () {
    ...
    await expect(rentalityGateway.connect(guest).invest(1, 10000, { value: 10000 })).to.not.reverted
  ...
  it('Possible to create car after investment', async function () {
    ...
    await expect(rentalityGateway.connect(guest).invest(1, fromUsd[0], { value: fromUsd[0] })).to.not.reverted
...
10
  ...
  it('Happy case with investors car', async function () {
    ...
    await expect(rentalityGateway.connect(guest).invest(1, mockCarInvestment.priceInCurrency, { value: mockCarInvestment.priceInCurrency })).to.not.reverted
...
fee: 0
    ...
    await expect(rentalityGateway.connect(guest).claimAllMy(1)).to.changeEtherBalance(guest, 7200000000000)
  })
...
fee: 0
    ...
    await expect(rentalityGateway.connect(guest).claimAllMy(1)).to.changeEtherBalance(guest, 7200000000000)
    await expect(rentalityGateway.connect(guest).claimAllMy(1)).to.to.changeEtherBalance(guest, 0)
  })
  it('Calculation test: 1 investor has 2 investments', async function () {
    ...
    let fromUsd = await rentalityCurrencyConverter.getFromUsdCentsLatest(ethToken, mockCarInvestment.priceInCurrency)
    await expect(rentalityGateway.connect(guest).invest(1, fromUsd[0] / BigInt(2), { value: fromUsd[0] / BigInt(2) })).to.not.reverted
...
" ",
    ...
    await expect(rentalityGateway.connect(guest).claimAllMy(1)).to.changeEtherBalance(
      guest,
...
" ",
  ...
  it('Calculation test: 3 investors has 1 investments and 3 finished trips', async function () {
    ...
    await expect(rentalityGateway.connect(anonymous).invest(1, fromUsd[0] / BigInt(4), { value: fromUsd[0] / BigInt(4) })).to.not.reverted

    await expect(rentalityGateway.connect(manager).invest(1, fromUsd[0] / BigInt(4), { value: fromUsd[0] / BigInt(4) })).to.not.reverted
...
fee: 0
    ...
    const investments1 = await rentalityGateway.connect(anonymous).getAllInvestments()
    await expect(rentalityGateway.connect(guest).claimAllMy(1)).to.changeEtherBalance(
      guest,
      ((priceForDayInETH[0] - priceForDayInETH[0] / BigInt(10)) * BigInt(3)) / BigInt(2)
    )

    const investments = await rentalityGateway.connect(anonymous).getAllInvestments()
    await expect(rentalityGateway.connect(anonymous).claimAllMy(1)).to.changeEtherBalance(
      anonymous,
      ((priceForDayInETH[0] - priceForDayInETH[0] / BigInt(10)) * BigInt(3)) / BigInt(4)
    )

    await expect(rentalityGateway.connect(manager).claimAllMy(1)).to.changeEtherBalance(
      manager,
      ((priceForDayInETH[0] - priceForDayInETH[0] / BigInt(10)) * BigInt(3)) / BigInt(4)
    )
  })
...
" ",
    ...
    let investments = await rentalityGateway.connect(guest).getAllInvestments()
    ...
    await expect(rentalityGateway.connect(guest).claimAllMy(1)).to.changeEtherBalance(guest, 0)
    await expect(rentalityGateway.connect(anonymous).claimAllMy(1)).to.to.changeEtherBalance(
      anonymous,
  ...
  it('Investment listing', async function () {
    ...
    await expect(await rentalityGateway.connect(host).createCarInvestment(mockCarInvestment, 'name',ethToken)).to.not
      .reverted
      await expect(rentalityGateway.connect(host).changeListingStatus(1)).to.not.reverted
      await expect(rentalityGateway.connect(admin).changeListingStatus(1)).to.be.reverted
      let investment = await rentalityGateway.connect(host).getAllInvestments()
      assert.equal(investment.length, 1)
      investment = await rentalityGateway.connect(admin).getAllInvestments()
      assert.equal(investment.length, 0)
      await expect(rentalityGateway.connect(host).changeListingStatus(1)).to.not.reverted

      investment = await rentalityGateway.connect(guest).getAllInvestments()
      assert.equal(investment.length, 1)
