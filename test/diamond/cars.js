...
describe.only('RentalityGateway: car', function () {
  ...
  it('should allow only host to update car info', async function () {
    ...
    ).to.be.revertedWith('Only the owner of the car can update car info')
  ...
  it('Should return public dto', async function () {
    ...
    await rentalityGateway.connect(host).burnCar(3)
    const hostCars = await rentalityGateway.getCarsOfHost(host.address)
    ...
    await rentalityGateway.connect(guest).burnCar(6)
    await expect(
      await rentalityGateway
        .connect(guest)
    ...
    const guestCars = await rentalityGateway.getCarsOfHost(guest.address)
  ...
  it('check available cars', async function () {
    ...
    const rentalityPaymentService = await ethers.getContractAt('RentalityPaymentsServiceFacet',await rentalityGateway.getAddress())
    const taxesService = await ethers.getContractAt('RentalityTaxesFacet',await rentalityGateway.getAddress())
    ...
    let totalTaxes = await taxesService.calculateTaxes(1, 1, priceWithDiscount)
