...
describe('RentalityGateway: trips', function () {
  ...
  it('createTripRequest', async function () {
    ...
    ).to.changeEtherBalances([guest, rentalityGateway], [-result.totalPrice, result.totalPrice])
  })

  it('Return valid trip data', async function () {
    const mockCreateCarRequest = await getMockCarRequest(0, await rentalityLocationVerifier.getAddress(), admin)
    ...
    expect(trip.paymentInfo).to.deep.equal(
      [
        1n,
        guest.address,
        await rentalityGateway.getAddress(),
  ...
  it('Return valid fuel prices', async function () {
    ...
    ).to.changeEtherBalances([guest, rentalityGateway], [-resultPatrol.totalPrice, resultPatrol.totalPrice])
  ...
  it('Host can not create trip request for own car ', async function () {
    ...
    const paymentService = await ethers.getContractAt('RentalityPaymentsServiceFacet', await rentalityGateway.getAddress())
  ...
  it('Happy case', async function () {
    ...
    const converter = await ethers.getContractAt('RentalityCurrencyConverterFacet',await rentalityGateway.getAddress())
    const depositValue = await converter.getFromUsd(
      ethToken,
      request.securityDepositPerTripInUsdCents,
      ethToCurrencyRate,
  ...
  it('Return correct total trip price', async function () {
    const mockCreateCarRequest = await getMockCarRequest(1, await rentalityLocationVerifier.getAddress(), admin)
    await expect(rentalityGateway.connect(host).addCar(mockCreateCarRequest)).not.to.be.reverted
    const myCars = await rentalityGateway.connect(host).getMyCars()
    expect(myCars.length).to.equal(1)
  ...
  it('Return in reject same value as in trip start', async function () {
    ...
    ).to.changeEtherBalances([guest, rentalityGateway], [-value, value])
  ...
  it('Get only host trips', async function () {
    ...
    const mockCreateCarRequest2 = await getMockCarRequest(1, await rentalityLocationVerifier.getAddress(), admin)
    ...
    ).to.changeEtherBalances([host, rentalityGateway], [-result.totalPrice, result.totalPrice])
