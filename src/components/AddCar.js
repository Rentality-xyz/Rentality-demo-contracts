...
export default function AddCar () {
    ...
    async function uploadMetadataToIPFS() {
        const {name, model, description, price} = formParams;
        //Make sure that none of the fields are empty
        if( !name || !model || !description || !price || !fileURL)
            return;

        const nftJSON = {
            name, model, description, price, image: fileURL
        }
    ...
    return (
        ...
        <div className="flex flex-col place-items-center mt-10" id="nftForm">
            ...
            <h3 className="text-center font-bold text-purple-500 mb-8">Upload your car to Rentality</h3>
                ...
                <button onClick={listCar} className="font-bold mt-10 w-full bg-purple-500 text-white rounded p-2 shadow-lg">
                    List car
                </button>
            </form>
        </div>
