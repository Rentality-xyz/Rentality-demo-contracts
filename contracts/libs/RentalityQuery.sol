...
library RentalityQuery {
    ...
    function searchAvailableCarsForUser(
        ...
        Schemas.DeliveryPrices memory deliveryPrices = RentalityCarDelivery(deliveryServiceAddress).getUserDeliveryPrices(user);
        for (uint i = 0; i < resultCount; i++) {
            ...
            if (bytes(locations.pickUpLat).length != 0) {
                deliveryFee = RentalityCarDelivery(deliveryServiceAddress).
