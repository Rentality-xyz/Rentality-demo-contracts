...
library RentalityQuery {
    ...
    function searchAvailableCarsForUser(
        ...
        for (uint i = 0; i < resultCount; i++) {
            ...
            result[i].geoData.locationLatitude = IRentalityGeoService(carService.getGeoServiceAddress()).getCarLocationLatitude(temp[i].carId);
            result[i].geoData.locationLongitude = IRentalityGeoService(carService.getGeoServiceAddress()).getCarLocationLongitude(temp[i].carId);
            result[i].geoData.timeZoneId = IRentalityGeoService(carService.getGeoServiceAddress()).getCarTimeZoneId(temp[i].carId);
