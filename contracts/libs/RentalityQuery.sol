...
library RentalityQuery {
    ...
    function getTripsByGuest(address tripServiceAddress, address userService, address guest) public view returns (Schemas.TripWithPhotoURL[] memory) {
        RentalityTripService tripService = RentalityTripService(tripServiceAddress);
        uint itemCount = 0;
        ...
        for (uint i = 1; i <= tripService.totalTripCount(); i++) {
            if (tripService.getTrip(i).guest == guest) {
                ...
                result[currentIndex].hostPhotoUrl = RentalityUserService(userService).
                    getKYCInfo(currentItem.host).
                    profilePhoto;
    ...
    function getTripsByHost(address tripServiceAddress, address userService, address host) public view returns (Schemas.TripWithPhotoURL[] memory) {
        RentalityTripService tripService = RentalityTripService(tripServiceAddress);
        uint itemCount = 0;
