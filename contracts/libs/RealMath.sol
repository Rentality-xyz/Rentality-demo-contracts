...
library RealMath {
  ...
  function RE_LN() public pure returns (int128) {
   ...
   */
  function toReal(int88 _ipart) public pure returns (int128) {
   ...
   */
  function hibit(uint128 val) public pure returns (uint128) {
   ...
   */
  function findbit(uint128 val) public pure returns (uint8 index) {
    index = 0;
   ...
   */
  function rescale(int128 real_arg) public pure returns (int128 real_scaled, int88 shift) {
    if (real_arg <= 0) {
      // Not in domain!
      revert();
    }
  ...
  function sqrt1(uint128 x) public pure returns (uint128 y) {
    uint128 z = (x + 1) / 2;
    y = x;
    while (z < y) {
      y = z;
