...
contract RentalityPlatform is UUPSOwnable {
    ...
    function createTripRequestWithDelivery(Schemas.CreateTripRequestWithDelivery memory request) public payable {
        ...
        bytes32 pickUpHash = IRentalityGeoService(addresses.carService.getGeoServiceAddress()).createLocationInfo(request.pickUpInfo.locationInfo);
        bytes32 returnHash = IRentalityGeoService(addresses.carService.getGeoServiceAddress()).createLocationInfo(request.returnInfo.locationInfo);
        _createTripRequest(
            request.currencyType,
            request.carId,
            request.startDateTime,
    ...
    function createTripRequest(Schemas.CreateTripRequest memory request) public payable {
        ...
        _createTripRequest(request.currencyType, request.carId, request.startDateTime, request.endDateTime, 0,bytes32(""), bytes32(""));
    }
    ...
    function rejectTripRequest(uint256 tripId) public {
        ...
        (uint valueToReturnInUsdCents, uint valueToReturnInToken) =
                                addresses.currencyConverterService.calculateTripReject(trip.paymentInfo);
        bool successGuest;
    ...
    function _finishTrip(uint256 tripId) internal {
        ...
        (uint valueToHost, uint valueToGuest, uint valueToHostInUsdCents, uint valueToGuestInUsdCents) =
                                addresses.currencyConverterService.calculateTripFinsish(trip.paymentInfo, rentalityFee);
        bool successHost;
        bool successGuest;
        if (addresses.currencyConverterService.isETH(trip.paymentInfo.currencyType)) {
    ...
    function updateCarInfo(Schemas.UpdateCarInfoRequest memory request) public {
        ...
        return addresses.carService.updateCarInfo(request,  IRentalityGeoService(addresses.carService.getGeoServiceAddress()).getLocationInfo(bytes32("")),string (""));
