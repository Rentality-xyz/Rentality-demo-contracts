...
interface IRentalityInvestmentFacet {
  ...
  function createCarInvestment(Schemas.CarInvestment memory car, string memory name_, address currency) external payable;
  function claimAndCreatePool(uint carId,Schemas.CreateCarRequest memory createCarRequest) external payable;
