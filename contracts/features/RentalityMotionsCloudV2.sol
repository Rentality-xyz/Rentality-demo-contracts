...
contract RentalityMotionsCloudV2 is UUPSAccess, EIP712Upgradeable {
  uint private caseId;
  ...
  function getCasesByTripId(uint tripId) public view returns (Schemas.MotionCloudCaseDTO[] memory motionCloudCases) {
    Schemas.CaseInfo[] memory tripCases = tripIdsToCaseInfos[tripId];

    motionCloudCases = new Schemas.MotionCloudCaseDTO[](tripCases.length);
    for (uint i = 0; i < tripCases.length; i++) {
      bytes32 hash = keccak256(abi.encodePacked(tripCases[i].caseToken));
      motionCloudCases[i] = Schemas.MotionCloudCaseDTO(
        tripCases[i].caseId,
  ...
  function initialize(address _userService) public initializer {
    ...
    __EIP712_init('RentalityMotionsCloud', '1');
  }
}
