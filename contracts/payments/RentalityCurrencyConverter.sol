...
contract RentalityCurrencyConverter is Initializable, UUPSAccess {
    ...
    function calculateValueWithFee(address ty, uint value, uint commission, int256 rate, uint8 dec) public view returns (uint, uint) {

        uint256 valueToPay = getFromUsd(
            ty,
            value + commission,
            rate,
            dec
        ...
        uint256 feeInCurrency = getFromUsd(
            ty,
            commission,
            rate,
            dec
    ...
    function calculateTripFinsish(Schemas.PaymentInfo memory paymentInfo, uint256 rentalityFee) public view returns (uint, uint, uint, uint)  {
