...
library RentalityUtils {
    ...
    function fetchAvailableCarsForUser(
        ...
        for (uint i = 0; i < carService.totalSupply(); i++) {
            uint currentId = i + 1;
            if (carService.isCarAvailableForUser(currentId, user, searchCarParams)) {
                itemCount += 1;
            }
        }
        ...
        for (uint i = 0; i < carService.totalSupply(); i++) {
            uint currentId = i + 1;
            if (carService.isCarAvailableForUser(currentId, user, searchCarParams)) {
                Schemas.CarInfo memory currentItem = carService.getCarInfoById(currentId);
                result[currentIndex] = currentItem;
                currentIndex += 1;
            }
        }
