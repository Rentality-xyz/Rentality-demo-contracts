...
contract RentalityTripService is ARentalityEventManager {

    function createTripRequestWithDelivery(
    Schemas.CreateTripRequestWithDelivery memory request,
    string memory promo
) public payable {
    (uint64 pickUp, uint64 dropOf) = RentalityTripsHelper.calculateDelivery(request);
  ...
  function createNewTrip(
    ...
    RentalityTripsHelper.validateTripRequest(
        currencyType,
    ...
    (
        ...
        uint hostEarnings,
        bool usePromo
    ) = RentalityTripsHelper.createPaymentInfo(
        params.request.carId,
        currencyType,
        params.pickUp,
        params.dropOf,
